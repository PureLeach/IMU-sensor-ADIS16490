...
spi=spidev.SpiDev()             #Cоздаем объект SPI
...
def SpiWrite(spi,reg,data):     #Функция записи данных по SPI
    send=[0]*2                  #Создаём список из двух элементов
    send[0]=0x80|reg            #В 0 ячейку списка записываем адрес, который указываем в параметре reg и с помощью лог.ИЛИ укаываем старший бит на запись            
    send[1]=data                #В 1 ячейку отправляем данные которые нужно записать
    spi.writebytes(send)        #Отпровляем байты по шине SPI
